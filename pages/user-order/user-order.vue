<template>
    <view :class="theme_view" class="order-page-points">
        <!-- 自定义金色导航 -->
        <view class="order-nav-bar">
            <view :style="'height:' + status_bar_height + 'px;'"></view>
            <view class="order-nav-content flex-row align-c">
                <view class="order-nav-back flex-row align-c" @tap="back_event">
                    <view class="order-nav-arrow"></view>
                    <text class="order-nav-back-text">{{ $t('index-points.index-points.back') }}</text>
                </view>
                <text class="order-nav-title">{{ $t('index-points.index-points.my_orders') }}</text>
            </view>
            <!-- 状态 Tab -->
            <view class="order-tabs flex-row">
                <view v-for="(item, index) in nav_status_list" :key="index" class="order-tab flex-1 tc pr" :class="nav_status_index == index ? 'order-tab-active' : ''" :data-index="index" @tap="nav_event">
                    <text>{{ item.name }}</text>
                    <view v-if="nav_status_index == index" class="order-tab-line"></view>
                </view>
            </view>
        </view>
        <!-- 订单列表 -->
            ...
            <view :class="nav_status_index == 1 && order_select_ids.length > 0 && home_is_enable_order_bulk_pay == 1 ? 'page-bottom-fixed' : ''">
                <view v-if="data_list.length > 0" class="padding-horizontal-main padding-top-main">
                    <view v-for="(item, index) in data_list" :key="index" class="list-item padding-horizontal-main padding-top-main border-radius-main bg-white oh spacing-mb">
                        <!-- 订单头：日期 + 状态 -->
                        <view class="order-head flex-row align-c jc-sb">
                            <view class="flex-row align-c">
                                <view v-if="nav_status_index == 1 && home_is_enable_order_bulk_pay == 1" :data-price="item.total_price" :data-oid="item.id" :data-payment="item.payment_id" class="margin-right-sm" @tap="selected_event">
                                    <iconfont :name="'icon-'+((order_select_ids.indexOf(item.id) != -1) ? 'selected-solid' : 'not-selected')" size="36rpx" :color="(order_select_ids.indexOf(item.id) != -1) ? theme_color : '#999'"></iconfont>
                                </view>
                                <text class="order-date">{{ item.add_time_time || item.add_time || item.warehouse_name }}</text>
                            </view>
                            <text class="order-status-name">{{ item.status_name }}<text v-if="(item.is_under_line_text || null) != null">（{{ item.is_under_line_text }}）</text></text>
                        </view>
                        <!-- 商品列表 -->
                        <view v-for="(detail, di) in item.items" :key="di" class="order-goods-row flex-row pr" :data-value="'/pages/user-order-detail/user-order-detail?id=' + item.id" @tap="url_event">
                            <image class="order-goods-img" :src="detail.images" mode="aspectFill"></image>
                            <view class="order-goods-info flex-1">
                                <view class="order-goods-title multi-text">{{ detail.title }}</view>
                                <view v-if="detail.spec != null" class="order-goods-spec single-text">
                                    <block v-for="(sv, si) in detail.spec" :key="si"><text v-if="si > 0">；</text><text>{{ sv.value }}</text></block>
                                </view>
                            </view>
                            <view class="order-goods-right tr">
                                <view class="order-goods-price">{{ item.currency_data.currency_symbol }}{{ detail.price }}</view>
                                <view class="order-goods-num">x{{ detail.buy_number }}</view>
                            </view>
                            <view v-if="item.is_can_launch_aftersale == 1 && (detail.orderaftersale_btn_text || null) != null" class="orderaftersale-btn-text pa" @tap.stop="orderaftersale_event" :data-oid="item.id" :data-did="detail.id">{{ detail.orderaftersale_btn_text }}</view>
                        </view>
                        <!-- 合计 -->
                        <view class="order-total tr">
                            <text class="order-total-label">{{$t('user-order-detail.user-order-detail.423rmr')}}</text><text class="order-total-num">{{ item.buy_number_count }}</text><text class="order-total-label">{{$t('user-order-detail.user-order-detail.41ty94')}}</text><text class="order-total-points">{{ item.currency_data.currency_symbol }}{{ item.total_price }}</text>
                        </view>
                        <view
                            v-if="
                                (

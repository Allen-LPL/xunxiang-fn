                        ...
                        <view v-if="home_use_multilingual_status == 1 && current_opt_form != 'bind_platform'" class="flex-row tr padding-top-sm padding-horizontal-lg padding-bottom-lg margin-bottom-xl">
                            <view class="flex-1 cr-base" @tap="open_language_event">
                                <view class="pr top-sm margin-right-sm dis-inline-block va-m">
                                    <iconfont name="icon-language-global" size="32rpx"></iconfont>
                                </view>
                                <text class="va-m text-size-sm">{{ language }}</text>
                                <view class="margin-left-sm dis-inline-block va-m">
                                    <iconfont name="icon-arrow-down-solid" size="16rpx"></iconfont>
                                </view>
                            </view>
                        </view>
...
<script>
    ...
    export default {
        ...
        onLoad(params) {
            ...
            // 强制刷新一次远程配置（清掉旧缓存→拉新配置→回调重新初始化），确保后台改过的开关（如登录图片验证码）能立即生效
            this.force_config_refresh();
        },
        ...
        methods: {
            ...
            // 强制刷新远程配置（先清掉本地配置缓存，避免用旧值打底产生竞态；远程返回写回缓存后再回调重新初始化）
            // 解决：后台修改了配置（如登录图片验证码开关）后，前端因读到旧缓存而不生效的问题
            force_config_refresh() {
                if (this.is_config_refreshing) {
                    return false;
                }
                this.is_config_refreshing = true;
                uni.removeStorageSync(app.globalData.data.cache_config_info_key);
                app.globalData.init_config(0, this, 'config_refresh_callback');
            },

            // 远程配置刷新完成回调（此时缓存已是最新，重新初始化表单与图片验证码）
            config_refresh_callback() {
                this.is_config_refreshing = false;
                this.init();
            },

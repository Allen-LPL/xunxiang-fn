...
- [x] **P1-17 投影清理（核心链路收尾）**：售后聊天状态点灰晕→0.06；page.css H5 返回钮 30%/底部浮条 0.16/右下浮动+号 #c4c4c4 光晕→全部 0.06；form-input 附件/上传件 0.1/灰晕 0.5→0.06；plugins.css 标签投影 60%→0.06；scan 取景框杂金 #d6b345→御金（遮罩 shadow 为功能性保留）。终断言：核心链路（pages 非插件+common+components）超标投影=0（豁免：autofill inset hack、0.03/0.04 低于上限者）。插件残留归 P2-1，diy float-window 服务端配置驱动归 P2-2。
- [x] **P1-18 全量复核轮**：复核发现并修复——.sales-price 全站价格 #E22C08→印章红（9 文件含支付/发票/签到选中态、buy/wallet 橙红渐变角标平涂）、buy.css 米色扩展块、theme.css 黄主题段大写 #FF0036×5→印章红、coupon 荧光橙渐变/描边、video 荧光红文字。终断言（排除 .old/uni_modules）：微信红=0、微信绿=0、荧光红橙系=0、米色底=0、金渐变=0、theme.css 渐变=0、核心域超标投影=0、宋体 13 文件、「」标题 5 处、token 完好。
